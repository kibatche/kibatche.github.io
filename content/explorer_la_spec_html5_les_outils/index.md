...
Ce fut le premier outil implémenté et c'est le premier que nous allons aborder.
...
Je dois avouer que je suis un grand admirateur de cette idée et du travail qui a été fourni pour la mettre en œuvre. Aussi, je ne tarirai pas d'éloges à son propos ! :)
### Le fonctionnement original de DOM-Explorer
#### Les pipes

L'application fonctionne avec ce que bitk nomme des `pipes` : ce sont des composants, nommés selon la bibliothèque de sanitisation mise en place ou le parseur utilisé.
...
Mais pas que ! On remarquera que nous avons parlé jusqu'à maintenant **d'un** nœud. Or, nous avons en avons systématiquement plusieurs dans un arbre : au moins le document + le *body* / *header*.
...
Parce que si les navigateurs nous permettent de créer dans une certaine mesure des nœuds arbitraires, les problèmes surviennent précisément quand on souhaite faire des choses qui sont interdites.
...
Or, dans l'idée de comparer un parseur *backend* à un oracle – le parseur de notre navigateur – opérer un parsing par le navigateur induirait quelque chose qu'on souhaite faire apostériori : le différentiel parseur *backend* / navigateur est celui qu'on souhaite *voir* lors d'un second parsing voulu, et non pas l'opérer en sous main à la construction d'un document avant son rendu.
...
De plus, `PHP` ne permet pas de sérialiser ses objets `C` comme ça : on doit construire cette sérialisation à la main. Et les équipes de PHP ont fait certains choix qui n'arrangent pas les choses : type de nœud qui a une signification différente en `PHP`, opacification du `content`, des templates et même absence complète de cet élément [dans toute sa spécifité](https://developer.mozilla.org/fr/docs/Web/HTML/Reference/Elements/template) en `PHP`.

En conséquence, il est bien plus simple et sûr de créer des interfaces miroir, dont on maîtrise la chaîne en entier, aussi bien dans les types des données transmises que dans leur quantité.
...
À l'image de ce que nous avons pour un parseur générique, deux fichiers sont créés : un pour la vue et l'autre pour la description du *pipe* et ses options.

Pour me faciliter la vie, j'ai réduit les options au strict minimum.

La différence majeure avec un parseur générique est la façon dont sont traités les nœuds.
...
Étant donné que nous construisons de toutes pièces un arbre, il nous faut donc des interfaces qui miment cet arbre, ainsi que les données qui s'y trouvent.

Pour nous, un nœud (et les variantes) sera réduit au minimum nécessaire : seules sont conservées les données effectivement exploitées par DOM-Explorer, à l'exception de certaines qui ont été laissées "au cas où".

Lexbor a en conséquence un fichier en plus `Lexbor.type.js`:
...
L'élément créé sera, comme on l'a vu, passé au sein d'un template, qui lui-même repasse la donnée à des templates spécialisés.

Étant donné que nous construisons tout de 0, nous devons également avoir nos propres templates, afin d'éviter d'une, le bazar, de deux, des effets de bords qui peuvent être néfastes.

En effet, nous ne retournons pas vraiment un nœud (le type `Node`), mais une interface miroir du nœud de la spécification (le type `LexborNode`). Comme décrit plus haut, les données que nous avons sont bien plus réduites et, de plus, nous avons besoin de connaître certaines choses en plus. 

Aussi les fichiers du dossier `Render` ont été dupliqués et adaptés à notre usage :
...
Rien à dire de spécial cependant, c'est juste un emballage. Côté DOM-Explorer, c'est très simple : il s'agit uniquement d'une chaîne qui est retournée !
...
Vous pouvez retrouver DOM-Explorer avec les fichiers *backend*, le binaire `PHP 8.4.18`, le binaire `lexbor 2.7.0` [ici](https://github.com/kibatche/Dom-Explorer-Spaghetti-Edition), bugs inclus ! Ne me remerciez pas, c'est naturel chez moi.

## `domuto`, un fork de `domato`

Dans l'introduction, j'ai présenté rapidement [domato](https://github.com/googleprojectzero/domato), un projet de Google Project Zero, écrit par Ivan Fratric, qui visait à tester les moteurs de rendu des navigateurs.
...
L'idée a été de créer de toutes pièces une grammaire de mutation, et de l'utiliser pour générer du HTML à partir d'elle. Ces portions générées sont ensuite envoyées au serveur `PHP`, et comparé à un oracle via Playwright : pour ma part, je l'ai utilisé avec le navigateur Firefox, mais ça pourrait aussi être chrome.

À chaque divergence, selon les options, un fichier HTML du *payload* est créé, et le tout est logué. On peut ensuite rejouer chacun de fichier sur DOM-Explorer ou passer l'ensemble à une IA pour voir exactement quel(s) mécanisme(s) ont pu créer la ou les divergences.

Ci-dessous, un diagramme représentatif du fonctionnement de `domuto` (généré par une IA) :

![](mermaid-diagram-2026-06-28-140543-DOMUTO.png)

Le programme original a largement été remanié/raccourci par claude et moi-même. Les options sont totalement différentes, ce qui était superfétatoire a été supprimé et seul a été conservé le cœur de `domato` : son moteur.

Vous pouvez retrouver le programme [ici](https://github.com/kibatche/domuto). Passons maintenant au plus intéressant : la grammaire mXSS !

### La grammaire mXSS

**La grammaire est le cœur du projet**. L'idée, comme précédemment écrit, est de générer des *payloads* mXSS à la volée. Et pour ce faire, nous avons besoin de deux choses :

1. Comprendre comment créer une grammaire dans `domato`
2. Faire une abstraction des règles HTML qui provoquent les mutations, reprendre des payloads existants.

#### Comment écrire une grammaire pour `domato`

L'écriture d'une grammaire pour le moteur de `domato` est un exercice un peu particulier. En effet, comme toute grammaire, elle impose des règles qu'il faut suivre et donc appréhender puis s'approprier.
...
3. Étant donné que `domuto` intègre dans sa construction de la récursivité, on met en place une règle de sauvegarde : si le nombre de récursions atteint une limite préalablement fixée, alors la récursion sera arrêtée par la génération d'un élément non récursif, ici :

```md
<mxss_payload nonrecursive=true p=0.1> = <lt>a<gt>SAFE FALLBACK AFTER TOO LONG RECURSION<lt>/a<gt>
```

En plus de l'option `nonrecursive` à `true`, cet élément possède une autre variable : `p`, pour *probability*. Cela veut dire que cet élément a une probabilité de 10 % d'être choisi parmi les autres éléments (qui se "partagent" le reste du gâteau).

4. Chacun des éléments `<mxss_[technique]>` est une définition de ladite technique. Par exemple, `<mxss_afe>` est construit comme suit :
...
On voit qu'ici, de nouveau, il s'agit d'un mix d'abstraction, qui renvoie à des définitions opérées ailleurs. On constate également qu'on retrouve l'élément pourtant plus généraliste `<mxss_payload>`, qui peut donc renvoyer à d'autres éléments de ce type par la suite, et être intégré au sein même d'une typologie particulière.
...
On peut constater que c'est un peu comme un jeu de brique à empiler, dans lequel on peut introduire plusieurs éléments propices au chaos et donc aux bugs : *payloads* matriochkas, références croisées, etc.

À partir de la spécification, on peut faire une abstraction d'une classe particulière : on sait par exemple que le mode `in foreign content` est "cassé" dès lors que le parseur rencontre certains éléments. On peut tous les mettre sous une même bannière, `<foreign_content_pop>` par exemple, et introduire ces éléments-là au sein de *payloads* qui seront construits aléatoirement, mais dans la contrainte que nous lui aurons donné :
...
Les limites sont assez claires : je suis parti de la spécification, de la connaissance que j'en ai ainsi que de divers *payloads* qu'on peut retrouver à droite et à gauche.

Il est sûr que je n'ai pas tout mis, de même qu'il se peut que mes abstractions soient incomplètes. Il s'agit avant tout d'un travail de découverte, et il me faudrait probablement plusieurs semaines supplémentaires ainsi qu'un autre angle d'attaque pour améliorer ce travail.

L'autre souci est la verbosité des *payloads*, et l'intégration de *payload* qui viennent de "régions" différentes de la spéc. C'est un réel souci pour la verbosité, car il est très fastidieux parfois de comprendre pourquoi il y a une différence entre deux arbres.
...
Eh oui, en écrivant cet article et en faisant de nouveaux tests, j'ai été particulièrement embêté par ces problématiques. Aussi, ce qu'il y avait à faire, quoique fastidieux, restait malgré tout "simple". Le cœur était là, il suffisait de séparer les différentes régions de la spéc dans des fichiers idoines.

J'ai donc demandé à l'IA d'opérer ce travail, et les résultats sont décevants. Effectivement, cela permet de tester chaque "partie" de façon indépendante, et quelque part d'opérer des tests unitaires. Or, il s'avère que les différents bugs trouvés sont apparus de manière contingente, dans un bordel indescriptible, et qu'ils concernent vraiment des choses très très spécifiques de la spéc, avec des erreurs de logiques subtiles dans l'implémentation.
...
J'ai longuement hésité à parler d'IA dans cet article : le sujet est présent partout, tout le temps. Cependant, il est certain que sur les quelques semaines de travail sur ce projet, j'ai acquis une certaine expérience de l'IA dans le cadre d'un projet qui vise à apprendre, et non pas à produire au sens économique du terme.

Je dois donc tout de même aborder très rapidement cette question.

Il est évident que je n'aurais jamais pu faire ce projet sans une IA, *aussi rapidement*. Le travail à abattre me semble réellement conséquent, et j'ai fait cela sur mon temps libre, et mon temps libre est limité. Et pourtant, ce travail, au moins sur la partie tests et essais, aura duré plusieurs mois. Je n'ai pas fait *que* cela de mon temps libre, mais tout de même.

Pour ma part, l'IA aura été une aide précieuse sur un point : écrire la documentation de mes trouvailles / essais / erreurs (une sorte de log géant qui mélange constats, hypothèses, compréhension que j'aie de telle ou telle chose).

![docs](docs.png)

Concernant le code, c'est plus mitigé. J'ai programmé la plupart du fork de DOM-Explorer (toute la partie front, backend, le plug-in lexbor, etc.), tandis que claude a fait le gros du travail sur *domuto*, grammaire exclue.

Le travail est toujours en cours, mais j'essaye autant que faire ce peut bien spécifier ce que fait l'IA de ce que je fais. Le souci étant généralement que, si le code fonctionne, leur structuration est vraiment catastrophique, que ce soit un modèle dit "frontière" ou non. Donc pour un sujet tel que celui-ci, qui est un peu du prototypage pour tester des idées, je pense que leur utilisation est satisfaisante.

Pour d'autres projets, qui demandent plus de discipline, il faut mettre en place des règles et une infrastructure qui les contraignent très fortement : c'est selon moi le seul mode satisfaisant avec ces machines.

En effet, travailler avec une IA est fatigant. Elle répète énormément, a des mécanismes d'attention qui sont parfois étranges et déstabilisants, elle utilise un vocabulaire auquel nous ne sommes pas forcément habitué.es – y compris si on est une lecteurice assidu.e – et n'a pas le talent que nous, humaines et humains, avons de bien considérer ce qui relève de l'important plutôt que de l'anecdotique.

Il faut donc vraiment passer du temps à faire son infra, par exemple en partant de [lifeOS](https://github.com/danielmiessler/LifeOS) de Daniel Miessler, qui permet d'avoir une bonne base. Pour ma part, j'ai largement modifié son fonctionnement, notamment pour qu'elle puisse adopter des postures différentes selon le travail qui lui est demandé.

Par exemple, les IA sont mauvaises en pédagogie. Si le pompage de tout internet et le vol à grande échelle opérés par les éditeurs de ces machines leur permet de contenir un savoir incommensurable, il n'en reste pas moins que ces machines **ne sont pas** des humains, ni des amis, ni des profs ni quoique ce soit se rapprochant de nous.

Elles traitent des informations et agissent en fonction de ces informations. Leur manière d'aborder le savoir est sans-pitié : la moindre erreur est sanctionnée de petites vexations qui nuisent à l'apprentissage. Eh oui, pensez à ça quelque secondes : comment réagiriez-vous si, à une erreur quelconque, votre prof vous disait :

> La moitié de ton raisonnement est bon, l'autre faux, et ce n'est pas pour la raison que tu penses.

Quoiqu'on puisse en dire, les IA manipulent avec excellence le langage naturel, et notre complexion naturelle, à nous, est d'y réagir, d'en être affecté [au sens spinoziste](https://spinozaetnous.org/wiki/Affect). C'est un mécanisme extrêmement puissant, dangereux même, déjà exploré – entre autres – dans le livre de Soshana Zuboff, *L'âge du capitalisme de surveillance* ([écoutez-la ici](https://www.radiofrance.fr/franceculture/podcasts/un-monde-connecte/shoshana-zuboff-et-le-capitalisme-de-surveillance-a-l-heure-de-l-ia-3483935)).

Il faut donc contraindre ces machines, pour qu'elles soient véritablement des aides et pas des nuisibles. Il faut être discipliné, pour vraiment apprendre, et ne pas tomber dans le sentiment de savoir, avoir l'arôme `Savoir ™`, sans le contenu. C'est à mon sens un enjeu énorme.

Je ne continuerai cependant pas sur le sujet dans cet article. Les liens entre apprentissage et IA méritent bien plus que quelques prises de position balancées comme ça dans un article sur les mutations dans le `DOM`. De plus, il existe énormément d'autres problématiques qu'il serait essentiel d'aborder, que ce soit sur l'écologie, la santé mentale, la souveraineté ou encore la protection de nos données et du savoir humain.
...
Nous avons pu voir rapidement dans cet article les outils qui ont été créés afin de continuer notre quête sur les mutations. Que ce soit le version Code Spaghetti de DOM-Explorer, ou Domuto, nous avons maintenant toutes les cartes en main pour nous attaquer au 4ᵉ et dernier article : quelques mutations trouvées dans lexbor. Et aussi, avec une question : Symfony HTML Sanitizer est-il vulnérable aux mutations ?
