+++
title = "Test"
slug = "apprendre_la_spec_html5_the_cool_way"
date = "2026-03-28"
template = "page.html"
+++

# Apprendre la spécification HTML5 : *The cool way*
...
En fin d'année 2024 et au début de l'année 2025, outre l'élection de Donald Trump, les saluts nazis de Musk, le pls généralisé dans que le monde compte de gens de gauche, il y eut deux articles de Kevin Mizu sur la librairie DOMPurify : [Exploring the DOMPurify library: Bypasses and Fixes (1/2)](https://mizu.re/post/exploring-the-dompurify-library-bypasses-and-fixes) et [Exploring the DOMPurify library: Hunting for Misconfigurations (2/2)](https://mizu.re/post/exploring-the-dompurify-library-hunting-for-misconfigurations).
...
Et en général, ces vulnérabilités surviennent via des mutations de l'arbre généré par le parseur HTML : le DOM. Dans le cadre de `DOMPurify` (côté client), cet arbre est créé par `DOMParseur`, la fonction générique présente dans tous les programmes prétendant implémenter la spécification `HTML`, par exemple les navigateurs et de façon plus générique les `user-agents`, les agents utilisateurs en français, selon le langage de whatwg.

Cette généricité permet donc à n'importe quel programme d'utiliser le standard HTML.

Et, si la plupart des recherches publiées sur le sujet des attaques XSS par mutations s'attachent à trouver des comportements problématiques sur des implémentation côté client - dont DOMPurify est sans conteste le graal - il existe pléthores d'implémentations côté serveur. [CONFUSION ENTRE DOMParser les librairies de sanitisation, à changer.]

Parmi elles, se trouvent des librairies `PHP`. La plus connue est `libxml2`, qui est un portage un peu bizarre de la spécification html5 à partir d'une librairie qui a été essentiellement créée pour parser du xml, comme son nom l'indique fort bien.

Cependant, `libxml2` a laissé place à une autre interface de parsing au sein de PHP : depuis PHP 8.4 c'est  le parseur `lexbor` qui est utilisé via l'API `DOM`. Ce parseur, créé en C par une seule personne, a pour prétention - sans que ce mot ait ici un sens péjoratif - d'implémenter la spécification HTML5 dans la globalité, et donc d'être *spec compliant*. Cela donne la possibilité aux programmes PHP d'utiliser une interface unifiée, débarrassée des problèmes inhérents à *libxml2*.

Sans aller plus loin pour l'instant sur ces histoires, il s'agit d'une entreprise gargantuesque : la spécification HTML5 porte sur plusieurs milliers de lignes, avec un langage abscons à de nombreux endroits, des ambiguïtés sur ce qu'un programme souhaitant l'implémenter doit faire, des machines à états extrêmement difficiles à comprendre par moment, de nombreuses définitions, des cas limites aux conséquences difficilement observables par la "simple" lecture de la spécification etc.
...
C'est un sujet qui pourrait paraître dépassé : quel est l'intérêt d'explorer les mécanismes de mutations sur un parseur HTML5 à l'heure de la toute puissance des sanitizers, du *hardening* opéré au sein des navgateurs afin de limiter au maximum les possibilités d'exploitation ?

D'ailleurs, en a-t-on quelque chose à foutre, nous, petites mains de la sécurité informatique qui vont tôt ou tard se faire remplacer par des machines surpuissantes ?

L'aquoibonisme ne serait-elle la meilleure posture à adopter ?

A ces questions je dirais : la volonté de savoir. 
...
Car malgré la complexité du sujet, il y avait un truc : ce qui était présenté par mizu était vraiment *cool* (oui juste ça c'est suffisant), mais en plus c'était intriguant. Et quoi de mieux pour étudier un sujet complexe que de le faire via les potentialités de subversion qu'il renferme ?

De mon point de vue, étudier une partie de la spécification HTML5 par ses abus est vraiment une façon excellente de le faire : il y a un côté amusant à la chose, un peu sorcier fou, à tester différentes charges utiles auxquelles on peut penser après la lecture de certaines parties de la spécification. Il était (et ça l'est toujours) inenvisageable pour moi de le faire de la façon conseillée par whatwg :
...
Car outre le fait que nous pouvons avoir d'autres choses à foutre que passer nos journées à lire une spécification de plusieurs milliers de pages, il s'agissait aussi de voir comment on pouvait *utiliser* ces abus : en effet, l'enjeu principal des mutations réside dans le contournement des mécanismes de protection, et ce n'est pas par la **seule** lecture qu'on peut tenter cela.

De plus, lire dans son entièreté quelque chose d'aussi énorme semble contre-productif. En effet, la spécification est connue pour contenir *certains*  passages qui peuvent occasionner des problèmes et, si d'autres peuvent éventuellement mener à certains comportements étranges, il s'agissait de *découvrir* la spécification HTML5, pas de la maîtriser.
...
On le verra plus tard, mais cette façon de présenter est un peu trompeuse : cet apprentissage démarré mi-février a en fait constamment dérivé vers plusieurs sujets, et ces sujets en eux-mêmes dépassent le cadre de la "simple" recherche de mutation (qui est tout sauf triviale). Je vais donc tenter d'aborder ces différents aspects tout du long de ces X articles. Cela concernera aussi bien l'outillage que la méthodologie ou encore l'utilisation de la dite "intelligence artificielle" dans le cadre d'une recherche à visée pédagogique.

Nous allons aborder un peu de théorie, l'outillage et la méthodologie de recherche, l'utilisation de la dite "Intelligence Artificielle" dans le cadre d'un apprentissage, puis différents résultats produits par la conjonction de ces différents éléments.

## Parti I : Un peu d'histoire et de théorie

Si lire la spécification dans son entier n'est selon moi pas utile, il n'en reste pas moins que sans idée du contenu de cette spécification, de la façon dont elle oriente les implémentations, on sera bien embêté pour comprendre quoique ce soit aux mutations. Le but de ces articles étant d'être les plus pédagogiques possible, nous allons tout d'abord voir ce que contient la fameuse spécification HTML5, évidemment de façon superficielle, et quels sont les zones d'intérêt de cette dernière.

## C'est quoi "HTML5"

Une réponse rapide serait : la suite de HTML4. Mais une réponse qui se rapprocherait plus de la vérité serait : une tentative de mettre un peu d'ordre dans le bazar incroyable que pouvait être les technologies utilisées sur les pages web au début des années 2000.
...
Il s'agit d'une spécification détaillée qu'on peut arbitrairement séparer en trois grandes parties :
...
La section la plus intéressante pour nous est donc celle qui concerne la syntaxe du HTML.

### La syntaxe HTML

Comme exposé plus haut, la section `Syntaxe HTML` est celle qui permet aux agents utilisateur d'implémenter la construction de l'arbre.

Pour faire simple, on peut voir le processus comme suit :

![Schéma Parsing.png](Schéma Parsing.png)
...
> [!note]
> Nous passons consciemment l'étape de pré-calcul. L'idée n'est pas de faire à la main tout l'algorithme de parsing, mais d'en présenter les grandes lignes sans sacrifier non plus certains détails.

C'est lors de cette "étape" que commence véritablement le travail de construction de l'arbre. Pour celles et ceux qui ont déjà eu l'occasion de faire leur propre shell, il s'agit d'un travail similaire : **sans tout détailler, chaque token va changer l'état de la machine à état, selon un mode d'insertion donné , selon une pile (*stack*) nommée "Pile des éléments ouverts" ([*stack of open elements*](https://html.spec.whatwg.org/multipage/parsing.html#the-stack-of-open-elements)) et selon une liste d'éléments de formatage actifs ([*Active Formatting Elements*](https://html.spec.whatwg.org/multipage/parsing.html#the-list-of-active-formatting-elements))** . L'agent utilisateur est également influencé dans son exécution par des drapeaux, que nous n'aborderons pas ici. Il existe également 

- [**Les États :**](https://html.spec.whatwg.org/multipage/parsing.html#tokenization)
 Il y a de très nombreux états. Parmi eux, on peut trouver le *Data State*, qui est l'état initial, le *RAWTEXT state* etc. Les états permettent au programme de **déterminer l'état suivant selon la valeur du token qui est lu**. Ces différents états ont des variables, notamment les modes d'insertions.
...
- **[Liste des éléments de formatage actif :](https://html.spec.whatwg.org/multipage/parsing.html#the-list-of-active-formatting-elements)**
...
> Il s'agit d'une application qui permet d'illustrer l'arbre HTML résultant d'une entrée utilisateur avec certains attributs utiles lors de l'exploration des mutations.
...
> [!warning]
> Cette section est particulièrement difficile à comprendre je trouve. Aussi je prends des raccourcis, et il est fort possible que des erreurs s'y trouvent.
...
Je dois avouer que je trouve ce passage particulièrement ardu. Comme pour l'nsemble de ces articles, il faut prendre ce que j'écris avec des pincettes.
...
![html_mal_formé.png](html_mal_formé.png)

On voit que l'élément `b` est fermé avant l'élément `i` qui se trouve pourtant après `b`.

la pile des éléments ouverts est `html`, `body`, `p`, `b` et `i`. La liste des éléments de formatage actifs contient `b` et `i`. 

L'agent utilisateur va donc reconstruire un arbre valide, qui deviendra *in fine* :

![Pasted image 20260406143600.png](Pasted image 20260406143600.png)

**Un nouvel élément `i` a été créé !!**

Est-ce de la magie ?

![medium_html.png](static/images/html_the_cool_way/medium_html.png)
...
2. Mais lorsque le parseur arrive à l'élément `</b>`, l'algorithme de réagencement est invoqué.

> [!note]
> Je vais être honnête : j'ai du mal à le comprendre. Il faudrait pour mieux le comprendre que j'implémente cette portion de la spécification au sein d'un programme, mais j'ai un gros problème : la flemme. Donc on va se contenter du résultat de cet algo.
...
On constate que la structure du DOM est inchangée mais que la liste des éléments de formatage actifs  contient désormais `i` seulement et que la pile des éléments ouverts ne possède plus que `html`, `body` et `p`. L'AAA a donc retiré ces éléments de cette liste, ils sont donc maintenant fermés.
...
Cet algorithme, un peu plus compréhensible soit dit en passant, permet de créer un nouvel élément équivalent à un élément de la liste des éléments de formatage actif si :

- La liste n'est pas vide
- L'élément traité n'est pas un marqueur (comme `template`) et n'est pas dans la pile des éléments ouverts
...
*In fine*, on se retrouve avec le résultat suivant, après avoir traité le reste des tokens :
